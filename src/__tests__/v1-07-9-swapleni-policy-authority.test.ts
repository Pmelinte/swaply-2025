...
describe("V1-07.9 Swapleni policy authority", () => {
  it("keeps numeric reward policies inactive until explicit configuration", () => {
    ...
    expect(migration).not.toContain("values ('story_publication'");
    expect(migration).not.toContain("values ('blog_contribution'");
  });
  ...
  it("supports only Story publication and approved Blog contribution sources", () => {
    ...
    expect(migration).toContain("Swapleni reward source is not eligible");
  ...
  it("uses replay-safe compensating reversals only after a source becomes ineligible", () => {
    ...
    expect(migration).toContain("reversal_row.reversal_of = v_original.id");
