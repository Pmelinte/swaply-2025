...
describe("Batch 66.8 Home drawer accessibility contract", () => {
  ...
  it("uses labeled heading and keyed list semantics without duplicate nav landmarks", () => {
    ...
    expect(homeDrawer).toContain("Children.toArray(children).map((child, index) => (");
    expect(homeDrawer).toContain("<li key={index}>{child}</li>");
