...
export async function GET(request: NextRequest) {
  ...
  if (swapId) query = query.eq("swap_id", swapId);
    ...
    query = query.or(
      `initiator_id.eq.${user.id},respondent_id.eq.${user.id}`,
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!swapId || !reason) {
    return NextResponse.json(
      { error: "swapId and reason are required" },
      { status: 400 },
    );
  }
  ...
  if (isSwapStatus(swap.status) && swap.status !== "disputed") {
    const transition = await transitionSwapStatusAuthoritatively({
      ...
      console.error("[disputes] swap transition failed:", error);
      return null;
    });
    ...
    if (
      ...
      transition.outcome !== "replayed"
    ) {
      console.error(
        "[disputes] swap transition rejected:",
        transition.outcome,
      );
    }
  }
  ...
  await db
    ...
    .then(({ error: notifErr }) => {
      if (notifErr) {
        console.error("[disputes] notification error:", notifErr.message);
      }
    });
