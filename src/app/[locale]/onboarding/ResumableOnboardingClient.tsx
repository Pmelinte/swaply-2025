...
function Choice({ title, values, selected, onClick }: any) { return <div><h2 className="mb-2 font-semibold">{title}</h2><div className="flex flex-wrap gap-2">{values.map((value: string) => <button type="button" key={value} onClick={() => onClick(value)} className={`rounded-full border px-3 py-2 text-sm capitalize ${Array.isArray(selected) ? selected.includes(value) : selected === value ? "bg-blue-600 text-white" : ""}`}>{value}</button>)}</div></div>; }
