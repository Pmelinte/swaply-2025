...
function SlotCard({
  ...
  const tags = item.userFinalTags?.length
    ? item.userFinalTags
    : item.aiSuggestedTags;

  return (
    <div
      className={`relative overflow-hidden rounded-xl border ${borderColor} ${bgColor} p-2`}
    >
      ...
      <p className="truncate text-[10px] text-zinc-500">
        {tCat(normalizeCategory(item.category))} &middot;{" "}
...
function ObjectCard({ item, mode }: { item: Item; mode: BrowseMode }) {
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center gap-2 text-xs text-zinc-500">
            <span className="rounded-full bg-zinc-100 px-2 py-0.5 dark:bg-zinc-700">
              {tCat(normalizeCategory(item.category))}
            </span>
  ...
  return (
    ...
    >
      <div
        className={`item-card__image relative aspect-[4/3] w-full overflow-hidden dark:bg-zinc-700 ${CAT[cat].placeholder}`}
      >
        <SafeImage
          src={item.photos?.[0] || NO_IMAGE_URL}
          alt={item.title}
        ...
        />
        <span
          className={`item-card__badge absolute left-2 top-2 rounded-full px-2 py-0.5 text-[10px] font-semibold backdrop-blur ${CAT[cat].badge}`}
        >
          {CAT[cat].icon}
        </span>
        {item.isBoosted && (
      ...
      <div className="item-card__body flex flex-1 flex-col p-3">
        ...
        <p className="mt-0.5">
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${CAT[cat].chip}`}
          >
            {tCat(normalizeCategory(item.category))}
          </span>
        </p>
...
export default function ObjectsPage() {
  ...
  const initialListingType: ListingType | "all" =
    initialTypeFromUrl === "property" ||
    initialTypeFromUrl === "service" ||
    initialTypeFromUrl === "object"
      ? initialTypeFromUrl
      : "all";
  ...
  const [sortMode, setSortMode] = useState<
    "newest" | "category" | "condition" | "popular" | "trusted"
  >(() => {
  ...
  useEffect(() => {
    ...
    if (next !== current) {
      router.replace(next ? `/objects?${next}` : "/objects", { scroll: false });
    }
  }, [
    categoryFilter,
    conditionFilter,
  ...
  const [wishSlots, setWishSlots] = useState<(Item | null)[]>([
    null,
    null,
    null,
  ]);
  ...
  const [wishHistory, setWishHistory] = useState<
    Array<{ item: Item; action: "left" | "right" }>
  >([]);
  ...
  const [offerSlots, setOfferSlots] = useState<(Item | null)[]>([
    null,
    null,
    null,
  ]);
  ...
  const wishCandidates = useMemo(() => {
    ...
    return otherItems.filter((i) => {
      ...
      if (
        ...
        (i.listingType ?? "object") !== listingTypeFilter
      )
        return false;
      return true;
    });
  ...
  const undoWishSwipe = useCallback(() => {
    ...
    if (last.action === "left") {
      setWishDismissed((prev) => {
        const n = new Set(prev);
        n.delete(last.item.id);
        return n;
      });
    } else {
      setWishSlots((prev) => {
  ...
  const filtered = useMemo(() => {
    ...
    if (listingTypeFilter !== "all") {
      result = result.filter(
        (i) => (i.listingType ?? "object") === listingTypeFilter,
      );
    }
    if (search.trim()) {
    ...
    if (valueFilter)
      result = result.filter((i) => i.perceivedValue === valueFilter);
      ...
      case "popular":
        ...
        result = boostFirst(
          [...result].sort((a, b) => {
            const aScore =
              (a.photos?.length ?? 0) * 2 +
              (a.userFinalTags?.length ?? 0) +
              (a.aiSuggestedTags?.length ?? 0);
            const bScore =
              (b.photos?.length ?? 0) * 2 +
              (b.userFinalTags?.length ?? 0) +
              (b.aiSuggestedTags?.length ?? 0);
        ...
        {!isSwipeView && showFilters && (
          <div className="mb-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/50">
            <div className="flex flex-wrap gap-4">
              <div>
                <p className="mb-1.5 text-xs font-semibold text-zinc-500 uppercase">
                  {t("filterCategory")}
                </p>
                ...
                <div className="flex gap-1.5">
                  ...
                  {(["new", "good", "used"] as const).map((cond) => (
                    <button
                      ...
                      onClick={() =>
                        setConditionFilter(
                          cond === conditionFilter ? null : cond,
                        )
                      }
                    ...
                    >
                      {t(
                        `condition${cond.charAt(0).toUpperCase() + cond.slice(1)}` as Parameters<
                          typeof t
                        >[0],
                      )}
                    </button>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  <button
                    ...
                    className={`rounded-full px-2.5 py-1 text-xs font-medium transition ${!valueFilter ? "bg-blue-600 text-white" : "bg-white text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-700 dark:text-zinc-300"}`}
                  >
                    {t("filterAll")}
                  </button>
                  {(["small", "medium", "large", "sentimental"] as const).map(
                    (value) => (
                      <button
                        key={value}
                        onClick={() =>
                          setValueFilter(value === valueFilter ? null : value)
                        }
                        className={`rounded-full px-2.5 py-1 text-xs font-medium transition ${valueFilter === value ? "bg-blue-600 text-white" : "bg-white text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-700 dark:text-zinc-300"}`}
                      >
                        {t(`value_${value}` as Parameters<typeof t>[0])}
                      </button>
                    ),
                  )}
                </div>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-2xl border-2 border-blue-300 bg-gradient-to-b from-blue-50 to-white p-4 shadow-sm dark:border-blue-800 dark:from-blue-950/40 dark:to-zinc-900">
              <div className="mb-3 flex items-center justify-between">
                ...
                <Pill color="blue">
                  {t("chosenDesired", { count: wishRightCount })}
                </Pill>
              </div>
            ...
            <div className="rounded-2xl border-2 border-emerald-300 bg-gradient-to-b from-emerald-50 to-white p-4 shadow-sm dark:border-emerald-800 dark:from-emerald-950/40 dark:to-zinc-900">
              <div className="mb-3 flex items-center justify-between">
                ...
                <Pill color="green">
                  {t("chosenOffered", { count: offerRightCount })}
                </Pill>
              </div>
            ...
            {/* ── Nudge banner ── */}
            {(wishRightCount >= MAX_RIGHT_SWIPES ||
              offerRightCount >= MAX_RIGHT_SWIPES) && (
                    ...
                    >
                      ...
                      {user ? (
                        <>
                          ...
                          >
                            <Heart
                              className={`h-3.5 w-3.5 ${favorites.has(item.id) ? "fill-red-500 text-red-500" : "text-zinc-400"}`}
                            />
                          </button>
                        </>
                      ) : (
                        <AuthGateModal
                          returnTo="/register?returnTo=/objects"
