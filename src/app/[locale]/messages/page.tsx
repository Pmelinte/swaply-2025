...
function PublicMessagesPreview({ locale }: { locale: string }) {
  const copy = getPublicCoreUi(locale);
  const loginUrl = `/${locale}/login?returnTo=/${locale}/messages`;

  return (
    <div className="space-y-6">
      <h1 className="sr-only">{copy.messagesTitle}</h1>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        {[copy.conversations, copy.secureChat, copy.exchangeTitle].map((label, index) => (
          <div
            key={label}
...
export default async function MessagesRoute() {
  ...
  if (!userId) {
    return <PublicMessagesPreview locale={locale} />;
  }
