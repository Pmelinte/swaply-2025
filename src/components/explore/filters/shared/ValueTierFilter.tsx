...
import type { ValueTier } from "@/lib/explore/exploreFilterTypes";

interface Props {
  value: ValueTier | null;
  onChange: (v: ValueTier | null) => void;
}

const TIERS: { emoji: string; value: ValueTier; labelKey: string; rangeKey: string }[] = [
...
export function ValueTierFilter({ value, onChange }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
      {TIERS.map((tier) => (
        ...
        >
          <span className="text-2xl mb-0.5">{tier.emoji}</span>
