...
do $$
begin
  if to_regprocedure('public.create_exchange_from_match_agreement_v1(uuid,integer)') is null then
    raise exception 'Batch 60 internal Exchange handoff implementation is missing';
  end if;
end;
$$;

-- Remove the duplicate index introduced by the superseding drift migration.
drop index if exists public.swaps_conversation_id_key;

-- Keep the canonical idempotency guard used by the reviewed Batch 60 branch.
create unique index if not exists swaps_conversation_id_unique
  on public.swaps (conversation_id)
  where conversation_id is not null;

-- The internal implementation remains server-only.
...
revoke all on function public.create_exchange_from_match_agreement_v1(uuid, integer)
  from authenticated;
grant execute on function public.create_exchange_from_match_agreement_v1(uuid, integer)
  to service_role;

-- Restore the reviewed, null-safe participant guard and the original response contract.
...
is 'Batch 60 restored hardened participant-only guard and explicit idempotent Exchange handoff after production migration drift.';
