...
-- Batch 62.3 authenticated closure found that a reused Review idempotency key
-- with a different payload was semantically a conflict but carried SQLSTATE
-- 22023, which the HTTP adapter correctly maps to 400. Use the same conflict
-- SQLSTATE already used for an immutable Review with changed content so the
-- canonical API returns HTTP 409 consistently.
...
revoke execute on function public.submit_swap_review_v1(
  uuid, integer, text, text[], text[], text
) from public, anon, service_role;
grant execute on function public.submit_swap_review_v1(
  uuid, integer, text, text[], text[], text
) to authenticated;

comment on function public.submit_swap_review_v1(
  uuid, integer, text, text[], text[], text
) is 'Canonical completed-Swap Review submission. Reused idempotency keys with changed payload return SQLSTATE 23505 / HTTP 409.';
