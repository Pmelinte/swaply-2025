...
begin
  ...
  v_response := pg_catalog.jsonb_build_object(
    'target_user_id', p_target_user_id,
    'blocked', found,
    'block', case when found then pg_catalog.to_jsonb(v_block) else null end,
    'refused_interest_count', v_refused_interest_count,
    'replayed', false,
...
drop trigger if exists enforce_unblocked_matching_interest_v1
  on public.matching_interests;
create trigger enforce_unblocked_matching_interest_v1
before insert or update on public.matching_interests
for each row execute function public.enforce_unblocked_matching_interest_v1();

create or replace function public.enforce_unblocked_conversation_v1()
returns trigger
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_pair_key text;
begin
  if cardinality(new.participant_ids) = 2 then
    v_user_a := new.participant_ids[1];
    v_user_b := new.participant_ids[2];
    v_pair_key := pg_catalog.least(v_user_a::text, v_user_b::text)
      || ':' || pg_catalog.greatest(v_user_a::text, v_user_b::text);

    perform pg_catalog.pg_advisory_xact_lock(
      pg_catalog.hashtextextended(v_pair_key, 0)
    );

    if private.is_blocked_pair_v1(v_user_a, v_user_b) then
      raise exception 'Blocked users cannot create conversations'
        using errcode = '42501';
    end if;
  end if;
...
revoke execute on function public.enforce_unblocked_conversation_v1()
  from public, anon, authenticated;

drop trigger if exists enforce_unblocked_conversation_v1
  on public.conversations;
create trigger enforce_unblocked_conversation_v1
before insert on public.conversations
for each row execute function public.enforce_unblocked_conversation_v1();

create or replace function public.enforce_unblocked_message_v1()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if new.sender_id is not null and new.recipient_id is not null then
    v_pair_key := pg_catalog.least(new.sender_id::text, new.recipient_id::text)
      || ':' || pg_catalog.greatest(new.sender_id::text, new.recipient_id::text);

    perform pg_catalog.pg_advisory_xact_lock(
      pg_catalog.hashtextextended(v_pair_key, 0)
    );

    if private.is_blocked_pair_v1(new.sender_id, new.recipient_id) then
...
revoke execute on function public.enforce_unblocked_message_v1()
  from public, anon, authenticated;

drop trigger if exists enforce_unblocked_message_v1 on public.messages;
create trigger enforce_unblocked_message_v1
before insert on public.messages
for each row execute function public.enforce_unblocked_message_v1();

create or replace function public.enforce_unblocked_swap_v1()
returns trigger
language plpgsql
security definer
set search_path = ''
...
declare
  v_pair_key text;
begin
  if new.requester_id is not null and new.responder_id is not null then
    v_pair_key := pg_catalog.least(new.requester_id::text, new.responder_id::text)
      || ':' || pg_catalog.greatest(new.requester_id::text, new.responder_id::text);

    perform pg_catalog.pg_advisory_xact_lock(
      pg_catalog.hashtextextended(v_pair_key, 0)
    );

    if private.is_blocked_pair_v1(new.requester_id, new.responder_id) then
      raise exception 'Blocked users cannot create new Swaps'
        using errcode = '42501';
    end if;
  end if;
...
revoke execute on function public.enforce_unblocked_swap_v1()
  from public, anon, authenticated;

drop trigger if exists enforce_unblocked_swap_v1 on public.swaps;
create trigger enforce_unblocked_swap_v1
before insert on public.swaps
for each row execute function public.enforce_unblocked_swap_v1();
