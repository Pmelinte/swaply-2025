...
--   * preserves the legacy authenticated UPDATE policy for the compatibility bridge;
--   * creates deterministic missing profiles and future Auth bootstrap;
--   * does not yet replace the public/participant profile projection;
--   * does not revoke direct UPDATE until authenticated RPC verification passes.
...
begin
  if not exists (
    ...
    alter table public.profiles
      ...
      check (primary_language = public.normalize_swaply_locale(primary_language));
  end if;
  ...
  if not exists (
    ...
    alter table public.profiles
      add constraint profiles_revision_positive check (profile_revision >= 1);
  end if;
...
comment on column public.profiles.auto_translate_messages is
  'Owner preference for automatic message translation; original text remains stored.';
comment on column public.profiles.show_original_language is
  'Owner preference for showing original content with a translation.';
...
-- Keep direct compatibility writes from changing server-owned fields while the
-- zero-downtime bridge remains enabled.
...
create table if not exists public.profile_update_idempotency (
  ...
  constraint profile_update_idempotency_key_length
    check (char_length(idempotency_key) between 8 and 120),
  constraint profile_update_idempotency_revision_positive
...
create index if not exists profile_update_idempotency_created_at_idx
  on public.profile_update_idempotency (created_at);
...
begin
  ...
  v_locale := coalesce(
    ...
    public.normalize_swaply_locale(v_auth.raw_user_meta_data ->> 'language'),
    'en'
  );
  ...
  insert into public.profiles (
    ...
    1,
    '{}'::jsonb,
    '{"publicProfile":true,"itemsVisibility":"public","showExactLocation":false,"showLastSeen":true}'::jsonb,
    '{"chat":true,"push":true,"email":true,"matches":true,"swapUpdates":true}'::jsonb,
    '{"notes":"","logistics":"flexible"}'::jsonb,
    '{"method":null,"passkeysEnabled":false,"twoFactorEnabled":false}'::jsonb,
    '{"tokens":0,"reputation":"starter","activeListings":0,"completedSwaps":0}'::jsonb
  )
...
begin
  ...
  insert into public.profiles (
    ...
    languages, primary_language, preferred_locale, profile_revision
  ) values (
    new.id,
    new.email,
...
revoke execute on function public.bootstrap_swaply_profile_from_auth()
  from public, anon, authenticated;

drop trigger if exists on_auth_user_created_profile on auth.users;
create trigger on_auth_user_created_profile
after insert on auth.users
for each row execute function public.bootstrap_swaply_profile_from_auth();

-- Additive repair for historical Auth users without a profile. Existing profile
