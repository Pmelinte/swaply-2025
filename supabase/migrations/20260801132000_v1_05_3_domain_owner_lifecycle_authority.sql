...
begin
  ...
  if v_domain = 'property' then
    ...
    if coalesce((v_listing ->> 'min_stay_days')::integer, 1) < 1
      or coalesce((v_listing ->> 'max_stay_days')::integer, 1)
        < coalesce((v_listing ->> 'min_stay_days')::integer, 1)
    then
    ...
    if coalesce((v_listing ->> 'capacity_total')::integer, 0) < 1
      ...
      or coalesce((v_listing ->> 'capacity_available')::integer, 0)
        > coalesce((v_listing ->> 'capacity_total')::integer, 0)
    then
  ...
  set title = case when v_item ? 'title' then btrim(v_item ->> 'title') else i.title end,
      description = case when v_item ? 'description' then nullif(btrim(v_item ->> 'description'), '') else i.description end,
      category_l1 = case when v_item ? 'category_l1' then nullif(btrim(v_item ->> 'category_l1'), '') else i.category_l1 end,
      category_l2 = case when v_item ? 'category_l2' then nullif(btrim(v_item ->> 'category_l2'), '') else i.category_l2 end,
      category_l3 = case when v_item ? 'category_l3' then nullif(btrim(v_item ->> 'category_l3'), '') else i.category_l3 end,
      category_path = case when v_item ? 'category_path' then nullif(btrim(v_item ->> 'category_path'), '') else i.category_path end,
      perceived_value_tier = case when v_item ? 'perceived_value_tier' then nullif(btrim(v_item ->> 'perceived_value_tier'), '') else i.perceived_value_tier end,
      swap_geo_preference = case when v_item ? 'swap_geo_preference' then coalesce(nullif(btrim(v_item ->> 'swap_geo_preference'), ''), 'regional') else i.swap_geo_preference end,
      swap_open_to = case when v_item ? 'swap_open_to' then array(select jsonb_array_elements_text(v_item -> 'swap_open_to')) else i.swap_open_to end,
      chain_swap_allowed = case when v_item ? 'chain_swap_allowed' then coalesce((v_item ->> 'chain_swap_allowed')::boolean, false) else i.chain_swap_allowed end,
      cross_category_swap = case when v_item ? 'cross_category_swap' then coalesce((v_item ->> 'cross_category_swap')::boolean, false) else i.cross_category_swap end,
      swap_partial_allowed = case when v_item ? 'swap_partial_allowed' then coalesce((v_item ->> 'swap_partial_allowed')::boolean, false) else i.swap_partial_allowed end,
      swap_partial_topup_eur = case when v_item ? 'swap_partial_topup_eur' then nullif(v_item ->> 'swap_partial_topup_eur', '')::numeric else i.swap_partial_topup_eur end,
      escrow_accepted = case when v_item ? 'escrow_accepted' then coalesce((v_item ->> 'escrow_accepted')::boolean, false) else i.escrow_accepted end,
      escrow_required = case when v_item ? 'escrow_required' then coalesce((v_item ->> 'escrow_required')::boolean, false) else i.escrow_required end,
      swap_wants_description = case when v_item ? 'swap_wants_description' then nullif(btrim(v_item ->> 'swap_wants_description'), '') else i.swap_wants_description end,
      swap_wants_type = case when v_item ? 'swap_wants_type' then array(select jsonb_array_elements_text(v_item -> 'swap_wants_type')) else i.swap_wants_type end,
      swap_wants_value_tier = case when v_item ? 'swap_wants_value_tier' then nullif(btrim(v_item ->> 'swap_wants_value_tier'), '') else i.swap_wants_value_tier end,
      location = case when v_item ? 'location' then nullif(btrim(v_item ->> 'location'), '') else i.location end,
      location_city = case when v_item ? 'location_city' then nullif(btrim(v_item ->> 'location_city'), '') else i.location_city end,
      location_country = case when v_item ? 'location_country' then nullif(upper(btrim(v_item ->> 'location_country')), '') else i.location_country end,
      images = case when v_item ? 'images' then coalesce(v_item -> 'images', '[]'::jsonb) else i.images end,
      image_url = case when v_item ? 'image_url' then nullif(btrim(v_item ->> 'image_url'), '') else i.image_url end,
...
begin
  ...
  if v_domain = 'event' and v_target_status = 'active' then
    ...
    where e.item_id = p_item_id and e.owner_id = v_actor;
  ...
  execute v_sql using v_target_status, p_item_id, v_actor into v_listing_id;
...
revoke all on function public.update_domain_listing_v1(text, uuid, jsonb, bigint, text)
  from public, anon;
grant execute on function public.update_domain_listing_v1(text, uuid, jsonb, bigint, text)
  to authenticated;

revoke all on function public.set_domain_listing_status_v1(text, uuid, text, bigint, text)
  from public, anon;
grant execute on function public.set_domain_listing_status_v1(text, uuid, text, bigint, text)
  to authenticated;

-- Domain listings are mutated exclusively by the security-definer authorities.
...
-- Owners may still read inactive rows for management; outsiders only see active rows.
...
-- Direct item writes remain available only for Objects. Domain items go through
-- the atomic functions above so items and the dedicated row cannot diverge.
...
-- The private projection is owner-readable only and never directly mutable.
...
comment on function public.update_domain_listing_v1(text, uuid, jsonb, bigint, text) is
  'V1-05.3 atomic, owner-bound, idempotent edit authority for property/service/event listings.';
comment on function public.set_domain_listing_status_v1(text, uuid, text, bigint, text) is
