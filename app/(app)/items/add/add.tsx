...
export default function AddItemPage() {
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      // ↑↑ Cloudinary upload (codul tău standard)
      ...
      const secureUrl = cloudinaryJson.secure_url;

      setImageUrl(secureUrl);
      setUploading(false);

      // 🔥 AI CLASSIFICATION — AUTOMAT DUPĂ UPLOAD
      setAiLoading(true);
    ...
    <div className="p-4 max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {uploading && <p className="text-blue-600 mt-2">Se încarcă imaginea...</p>}
          {aiLoading && <p className="text-purple-600 mt-1">AI analizează imaginea...</p>}
        ...
        <div>
          ...
          <input
            ...
            placeholder="electronics, fashion, furniture..."
          />
        </div>

        {/* SUBCATEGORY */}
        <div>
          <label className="block font-medium mb-1">Subcategorie</label>
          <input
            type="text"
            className="border rounded px-3 py-2 w-full"
            value={subcategoryId}
            onChange={(e) => setSubcategoryId(e.target.value)}
            placeholder="smartphones, laptops, shoes..."
          />
        </div>
