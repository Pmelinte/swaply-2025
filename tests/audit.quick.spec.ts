...
async function performLogin(page: Page, _context: BrowserContext, testInfo: TestInfo): Promise<boolean> {
  if (!TEST_EMAIL || !TEST_PASSWORD) {
    testInfo.annotations.push({
      type: 'warning',
      description: 'Missing login credentials for authenticated quick audit.',
    });
    return false;
  }
...
test.describe('swaply.world quick audit', () => {
  test('checks key routes for HTTP 200', async ({ page, context }, testInfo) => {
    ...
    for (const auditPage of QUICK_PAGES) {
    ...
    const jsonPath = path.join(resultsDir, 'swaply-quick-audit-results.json');
    await fs.promises.writeFile(jsonPath, JSON.stringify(results, null, 2), 'utf8');

    await testInfo.attach('swaply-quick-audit-results.json', {
      path: jsonPath,
