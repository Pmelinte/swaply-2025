...
const STORAGE_FILE = path.join(process.cwd(), "playwright", ".auth", "storage.json");
...
test.describe("swaply.world full route audit", () => {
  test("audits every route on desktop and mobile", async ({ browser }, testInfo) => {
    ...
    // Use pre-generated storage state when available (generated by Login step).
    // Falls back to inline Supabase auth if the file is missing or unreadable.
    let storageState: string | undefined;
    try {
      await fs.promises.access(STORAGE_FILE);
      storageState = STORAGE_FILE;
      console.log("[auth] Using pre-generated storage state from", STORAGE_FILE);
    } catch {
      console.log("[auth] No storage state file — will use inline Supabase auth.");
    }
    ...
    try {
      for (const viewportName of ["desktop", "mobile"] as const) {
        ...
        // Use inline auth only when the storage-state file was absent.
        const loggedIn = storageState
          ? true
          : await authenticateWithSupabase(page, context, testInfo);
      ...
      // Unexpected error mid-audit — record it and continue to write results.
      testInfo.annotations.push({
        type: "error",
        description: `Audit loop threw: ${(err as Error).message}`,
      });
    } finally {
      // Always write results.json — even if the loop threw or auth failed.
      await fs.promises.writeFile(
        ...
        console.error("[audit] Failed to write results.json:", writeErr.message);
      });
    }
